-- GPT Agents schema only (without auth.users modifications)
-- This migration adds support for managing custom GPT Agents

-- GPT Agents table
create table if not exists gpt_agent (
...
create table if not exists gpt_agent_usage (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists gpt_agent_config (
  id uuid primary key default gen_random_uuid(),
  agent_id uuid references gpt_agent(id) on delete cascade,
...
create index if not exists idx_gpt_agent_category on gpt_agent(category);
create index if not exists idx_gpt_agent_status on gpt_agent(status);
create index if not exists idx_gpt_agent_created_by on gpt_agent(created_by);
create index if not exists idx_gpt_agent_usage_agent_id on gpt_agent_usage(agent_id);
create index if not exists idx_gpt_agent_usage_user_id on gpt_agent_usage(user_id);
create index if not exists idx_gpt_agent_usage_created_at on gpt_agent_usage(created_at);
create index if not exists idx_gpt_agent_config_agent_id on gpt_agent_config(agent_id);
...
create policy if not exists "Allow all users to read active gpt_agents" on gpt_agent
  for select using (status = 'active');

-- Allow users to read their own agent usage
create policy if not exists "Allow users to read their own gpt_agent_usage" on gpt_agent_usage
  for select using (auth.uid() = user_id);

-- Allow users to insert their own agent usage
create policy if not exists "Allow users to insert their own gpt_agent_usage" on gpt_agent_usage
  for insert with check (auth.uid() = user_id);

-- Allow admins to manage all agents
create policy if not exists "Allow admins to manage all gpt_agents" on gpt_agent
  for all using (
    exists (
...
create policy if not exists "Allow admins to manage all gpt_agent_config" on gpt_agent_config
  for all using (
    exists (
      select 1 from app_user 
      where app_user.id = auth.uid() 
...
create policy if not exists "Allow admins to read all gpt_agent_usage" on gpt_agent_usage
  for select using (
    exists (
      select 1 from app_user 
      where app_user.id = auth.uid() 
...
drop trigger if exists update_agent_usage_count_trigger on gpt_agent_usage;
...
drop trigger if exists update_gpt_agent_updated_at_trigger on gpt_agent;
...
) as v(name, description, iframe_url, category, status, created_by)
where not exists (select 1 from gpt_agent where name = v.name);
...
select 
  ...
  '"gpt-4"'::jsonb
from gpt_agent 
where name = 'Workleap Content Assistant'
and not exists (select 1 from gpt_agent_config where agent_id = gpt_agent.id and config_key = 'model');
...
from gpt_agent 
where name = 'Workleap Content Assistant'
and not exists (select 1 from gpt_agent_config where agent_id = gpt_agent.id and config_key = 'temperature');
...
from gpt_agent 
where name = 'Workleap Content Assistant'
and not exists (select 1 from gpt_agent_config where agent_id = gpt_agent.id and config_key = 'max_tokens');
