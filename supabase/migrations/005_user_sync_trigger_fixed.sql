-- Migration: Create user sync trigger (Fixed)
...
-- Add helpful comments (only on functions we own)
COMMENT ON FUNCTION public.handle_new_user() IS 'Automatically creates app_user record when auth user is created';
COMMENT ON FUNCTION public.update_app_user_updated_at() IS 'Updates updated_at timestamp on app_user changes';
